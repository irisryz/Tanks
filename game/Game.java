...
public class Game implements ActionListener {
	...
	protected void awardPoints(double points, int playerIdx) {
    ...
    private void onLevelUpdate(double deltaTime, boolean firstUpdate) {
		...
		for (int i = 0; i < spawnCount; ++i) {
            ...
            String powerupType = (powerupRand < 0.2) ? "P" :
                     (powerupRand < 0.4) ? "R" :
                     (powerupRand < 0.6) ? "S" :
                     (powerupRand < 0.8) ? "H" : "T";        
            
            if (!playWithTargets) {
                powerupType = (powerupRand < 0.33) ? "P" : (powerupRand < 0.66) ? "S" : "H";
            }
        ...
        spawnCount = Math.max(spawnLandmineCount - Simulation.get().objectCount(Landmine.class), 0);
        
		for (int i = 0; i < spawnCount; ++i) {
            System.out.print("LANDMINE SPAWNED");
            Vec2 landmineLocation = pickSpawnLocation(6, 5, 5, 0.9, true, spawnInCenterOfField);
            Simulation.get().createLandmine(landmineLocation);
            System.out.print("LANDMINE SPAWNED");
